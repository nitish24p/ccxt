...
module.exports = class Exchange {
    ...
    constructor(userConfig = {}) {
        ...
        _Object$assign(this, functions, { encode: string => string, decode: string => string });
        ...
        // merge to this
        for (const [property, value] of _Object$entries(config)) this[property] = deepExtend(this[property], value);
        ...
        if (this.debug && journal) {
            journal(() => this.journal, this, _Object$keys(this.has));
        }
    }
    ...
    checkRequiredCredentials() {
        _Object$keys(this.requiredCredentials).map(key => {
    ...
    defineRestApi(api, methodName, options = {}) {
        ...
        for (const type of _Object$keys(api)) {
            for (const httpMethod of _Object$keys(api[type])) {
    ...
    setMarkets(markets, currencies = undefined) {
        let values = _Object$values(markets).map(market => deepExtend({
            'limits': this.limits,
        ...
        if (currencies) {
            ...
            const currencies = _Object$keys(groupedCurrencies).map(code => groupedCurrencies[code].reduce((previous, current) => previous.precision > current.precision ? previous : current, groupedCurrencies[code][0]));
    ...
    fetchMarkets() {
        return new _Promise((resolve, reject) => resolve(this.markets));
    }
    ...
    parseBidsAsks(bidasks, priceKey = 0, amountKey = 1) {
        return _Object$values(bidasks || []).map(bidask => this.parseBidAsk(bidask, priceKey, amountKey));
    }
    ...
    getCurrencyUsedOnOpenOrders(currency) {
        return _Object$values(this.orders).filter(order => order['status'] == 'open').reduce((total, order) => {
    ...
    parseBalance(balance) {
        ...
        const currencies = _Object$keys(this.omit(balance, 'info'));
        ...
        currencies.forEach(currency => {
            ...
            if (typeof balance[currency].used == 'undefined') {
                ...
                if (this.parseBalanceFromOpenOrders && 'open_orders' in balance['info']) {
                    ...
                    const cachedOrdersCount = _Object$values(this.orders).filter(order => order['status'] == 'open').length;
    ...
    parseTrades(trades, market = undefined, since = undefined, limit = undefined) {
        let result = _Object$values(trades).map(trade => this.parseTrade(trade, market));
        result = sortBy(result, 'timestamp', true);
        return this.filterBySinceLimit(result, since, limit);
    }

    parseOrders(orders, market = undefined, since = undefined, limit = undefined) {
        let result = _Object$values(orders).map(order => this.parseOrder(order, market));
        return this.filterBySinceLimit(result, since, limit);
    }
