...
const sleep = ms => new _Promise(resolve => setTimeout_safe(resolve, ms));
...
const timeout = (() => {
    var _ref = _asyncToGenerator(function* (ms, promise) {
        ...
        let clear = function () {};
        const timeout = new _Promise(function (resolve) {
            return clear = setTimeout_safe(resolve, ms);
        });

        try {
            return yield _Promise.race([promise, timeout.then(function () {
...
const keysort = object => {
    const result = {};
    _Object$keys(object).sort().forEach(key => result[key] = object[key]);
    return result;
};

const extend = (...args) => _Object$assign({}, ...args);
...
const groupBy = (array, key) => {
    const result = {};
    _Object$values(array).filter(entry => entry[key] != 'undefined').forEach(entry => {
...
const aggregate = function (bidasks) {
    ...
    return _Object$keys(result).map(price => [parseFloat(price), parseFloat(result[price])]);
};
...
const jwt = (request, secret, alg = 'HS256', hash = 'sha256') => {
    const encodedHeader = urlencodeBase64(stringToBase64(_JSON$stringify({ 'alg': alg, 'typ': 'JWT' }))),
